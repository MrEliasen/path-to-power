...
class EquippedItem extends React.Component {
    ...
    renderSpecialValue() {
        ...
        switch (item.type) {
            case 'weapon':
                if (item.subtype === 'ammo') {
                    specialValue = <span>
                        {item.stats.damage_bonus} DMG Bonus<br />
                        {item.stats.durability} Left
                    </span>;
                } else {
                    specialValue = `${item.stats.damage_min}-${item.stats.damage_max} DMG`;
                }
                break;

            case 'armour':
                specialValue = `${item.stats.damage_reduction} DMG Reduction`;
                break;
        ...
        return <div className="item-layer item-special">
            {specialValue}
        </div>;
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(dragSource('item', itemSource, collect)(EquippedItem));
